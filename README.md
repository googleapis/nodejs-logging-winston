...
If you wish to set the LogEntry `trace` and `spanId` properties with custom values, then set Winston metadata properties for `'logging.googleapis.com/trace'` and `'logging.googleapis.com/spanId'`, which is exported by this module as `LOGGING_TRACE_KEY` and `LOGGING_SPAN_KEY` respectively. For example:
